...
def generate_create_table_sql(schema, table_name) -> str:
    ...
    create_table_sql = f"""
    CREATE OR REPLACE TABLE {database}.{table_name} ({columns_sql}
    )
    USING iceberg
    PARTITIONED BY (day(DWH_kafka_timestamp))
    COMMENT 'Table description or task id'
    LOCATION 's3://warehouse/{database}/{table_name}/'
    TBLPROPERTIES(
...
ensure_iceberg_table_exists(
    table_name=table_name, message_schema=message_schema, database=database
)
...
df_deserialized.writeStream.format("iceberg").outputMode("append").trigger(
    once=True
).option("path", f"{database}.{table_name}").option("fanout-enabled", "true").option(
...
#     .option("checkpointLocation", checkpoint_location) \
#     .toTable(f"{database}.{table_name}")
#     .awaitTermination()
#     # .option("path", f"{database}.{table_name}") \
#     # .start()
